...
int tesselate(BSPBiQuadraticPatch *quadPatch)
{
	...
	{
		graphics->showErrorAndExit("BSP - Failed to allocate %zu bytes for tesselation", (bsp->tesselation + 1) * (bsp->tesselation + 1) * sizeof(BSPVertex));
	}
	...
	{
		graphics->showErrorAndExit("BSP - Failed to allocate %zu bytes for indices", bsp->tesselation * (bsp->tesselation + 1) * 2 * sizeof(unsigned int));
	}
	...
	{
		graphics->showErrorAndExit("BSP - Failed to allocate %zu bytes for trianglesPerRow", bsp->tesselation * sizeof(int));
	}

	if (quadPatch->rowIndexPointers == NULL)
	{
		graphics->showErrorAndExit("BSP - Failed to allocate %zu bytes for rowIndexPointers", bsp->tesselation * sizeof(unsigned int));
	}
...
int loadTextures(unsigned char *buffer, BSPLump *lumps)
{
	...
	{
		graphics->showErrorAndExit("BSP - Failed to allocate %zu bytes for textures", bsp->numOfTextures * sizeof(BSPTexture));
	}
	...
	{
		graphics->showErrorAndExit("BSP - Failed to allocate %zu bytes for texture validation", bsp->numOfTextures * sizeof(TextureSpec));
	}
...
int loadPatches(unsigned char *buffer, BSPLump *lumps)
{
	...
	if (bsp->numOfPatches > 0)
	{
		...
		if (bsp->patches == NULL)
		{
			printf("Failed to allocate %zu bytes for patches\n", bsp->numOfPatches * sizeof(BSPPatch));
